...
class RNN_train_fourier:
    ...
    def __call__(self, df):
        ...
        with torch.no_grad():
            ...
            y_test_pred = self.model.forecast_knownX(X_hist=X_train, X_future=X_test, T=T, sx=sx, sy=sy)

        train_residuals_matrix = np.array(train_residuals_matrix)

        epsilon_mean = np.mean(train_residuals_matrix, axis=0)
        epsilon_cov = np.cov(train_residuals_matrix, rowvar=False)
